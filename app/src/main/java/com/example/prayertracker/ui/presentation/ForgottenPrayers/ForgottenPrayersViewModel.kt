...
/* init {
     ...
     getDhuhrPrayerCount()
     getAsrPrayerCount()
     getMaghribPrayerCount()
     getMaghribPrayerCount()
 }

 fun getFajrPrayerCount() = viewModelScope.launch {
     loading.value = true
     getPrayerCountUseCase.execute(
         id = 1
     ).catch {
         Log.d(TAG, "Error in VM")
     }.collect { response ->
 ...
 fun getDhuhrPrayerCount() = viewModelScope.launch {
     loading.value = true
     getPrayerCountUseCase.execute(
         id = 1
     ).catch {
 ...
 fun getAsrPrayerCount() = viewModelScope.launch {
     loading.value = true
     getPrayerCountUseCase.execute(
         id = 1
     ).catch {
 ...
 fun getMaghribPrayerCount() = viewModelScope.launch {
     loading.value = true
     getPrayerCountUseCase.execute(
         id = 1
     ).catch {
 ...
 fun getIshaPrayerCount() = viewModelScope.launch {
     loading.value = true
     getPrayerCountUseCase.execute(
         id = 1
     ).catch {
...
/*fun getPrayerCount(id: Int) = viewModelScope.launch {
    loading.value = true
    getPrayerCountUseCase.execute(
        id = id
    ).catch {
        ...
        when (response) {
            ...
            is Resource.Success -> {
                prayerCount.value = response.data!!
                loading.value = false
            }
        }
    }
